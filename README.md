...
| `@Unique`                          | Copies the helper into the target under `__unique$<mixin>$<name>$<hash>`. Static helpers keep the original descriptor; instance helpers gain an `Object self` prepended slot (caller must `INVOKESTATIC` with the target instance). Instance helpers must be self-contained — references to the mixin class itself are rejected at transform. |
...
| `@Definition` + `@Expression` (v2) | DSL site selector for `@At(point = EXPRESSION)`. `@Definition(id, method/field)` aliases a JVM signature; `@Expression` selects matching instructions. Grammar: unchained `id(?, ?)` / `id`; chained `this.field`, `this.method(?)`; assignment `this.field = ?`. `?` placeholders bind positionally to handler params after `Object self` (each backwalks to a `*LOAD` predecessor — failed binds trigger `@Surrogate` fallback). v2 still matches a single instruction; multi-insn sequences and arithmetic are reserved for v3. |
