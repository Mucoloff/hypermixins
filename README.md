...
| `@Definition` + `@Expression` (v1) | DSL site selector for `@At(point = EXPRESSION)`. `@Definition(id, method/field)` aliases a JVM signature; `@Expression("id(?, ?)")` or `@Expression("id")` selects every matching INVOKE / FIELD instruction. Single-instruction expressions only; chained `.`, `this`, arithmetic, and `?` capture binding deferred.                              |
